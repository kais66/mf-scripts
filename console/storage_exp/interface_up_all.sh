...
for (( i=1; i<=7; i++)) do
  ssh root@node${nodes[$i]} "cd /root/scripts/exprmt/mobility-9nodes; nohup ./setup-ifs.sh $i"
done

sleep 15
# here depending on which channel the client is using, it connects to one of the 3 APs 
ssh root@node${nodes[9]} "cd /root/scripts/exprmt/mobility-9nodes; nohup ./setup-ifs.sh 9"
